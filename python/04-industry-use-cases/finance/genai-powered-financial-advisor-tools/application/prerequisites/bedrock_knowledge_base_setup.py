...
log_file_path = os.path.join(script_dir, 'knowledge_base_setup.log')
...
class KnowledgeBasesForAmazonBedrock:
    ...
    def create_or_retrieve_knowledge_base(
        ...
        if kb_id is not None:
            ...
            # Use shorter names for OpenSearch policies and collections (32 char limit)
            # Extract just the short name part before the first hyphen
            short_prefix = kb_name.split('-')[0][:8]  # Max 8 chars from prefix
            
            # OpenSearch Serverless collection name: 3-32 chars, lowercase, start with letter
            vector_store_name = f"{short_prefix}-{self.suffix}"  # e.g., "kb-20251002175214"
            
            # Policy names must be <= 32 chars
            encryption_policy_name = f"{short_prefix}-sp-{self.suffix}"
            network_policy_name = f"{short_prefix}-np-{self.suffix}"
            access_policy_name = f"{short_prefix}-ap-{self.suffix}"
            
            # Index name can be longer
            index_name = f"{short_prefix}-idx-{self.suffix}"
            
            # IAM role and policy names (no strict length limit)
            kb_execution_role_name = (
...
def update_config_file(config_path: str, kb_name: str, kb_id: str, bucket_name: str):
    """
    Update the YAML configuration file with knowledge base details
    Args:
    ...
    try:
        ...
        config['knowledge_base_name'] = kb_name
        config['knowledge_base_id'] = kb_id
        config['s3_bucket_name_for_kb'] = bucket_name
        
        logger.info(f"📝 Writing updated configuration to: {config_path}")
        with open(config_path, 'w') as file:
            yaml.dump(config, file, default_flow_style=False)
        
        logger.info(f"✅ Updated configuration file with:")
        logger.info(f"   - knowledge_base_name: {kb_name}")
        logger.info(f"   - knowledge_base_id: {kb_id}")
        logger.info(f"   - s3_bucket_name_for_kb: {bucket_name}")
    except Exception as e:
        logger.error(f"⚠️  Failed to update configuration file: {e}")
        raise
...
if __name__ == "__main__":
    ...
    if args.mode == "create":
        try:
            ...
            # Get the bucket name that was created
            bucket_name = kb.get_data_bucket_name()
            logger.info(f"✅ S3 Bucket: {bucket_name}")
            
            # Upload documents if path is specified
            if "kb_files_path" in data and data["kb_files_path"]:
                logger.info(f"📁 Uploading directory: {data['kb_files_path']}")
                kb.upload_directory(
                    f'{current_dir}/{data["kb_files_path"]}', bucket_name
                )
            else:
                # Upload client_meeting_note.txt from current directory
                notes_file = f"{current_dir}/client_meeting_note.txt"
                if os.path.exists(notes_file):
                    logger.info(f"📄 Uploading {notes_file} to {bucket_name}")
                    kb.s3_client.upload_file(notes_file, bucket_name, "client_meeting_note.txt")
                else:
                    logger.warning(f"⚠️  Warning: {notes_file} not found")
            ...
            update_config_file(config_path, kb_name, kb_id, bucket_name)
            
            # Optionally store in SSM Parameter Store
            try:
                logger.info("💾 Storing KB ID in SSM Parameter Store...")
                ...
                logger.info(f"✅ Stored KB ID in SSM Parameter Store: {kb_name}-id")
            except Exception as e:
                logger.warning(f"⚠️  Could not store in SSM Parameter Store: {e}")
        ...
        try:
            ...
            if not kb_name:
                logger.error("❌ No knowledge_base_name found in config file")
                sys.exit(1)
            
            logger.info(f"🗑️  Deleting Knowledge Base: {kb_name}")
            ...
            try:
                logger.info("🗑️  Deleting SSM parameter...")
                smm_client = boto3.client("ssm")
                smm_client.delete_parameter(Name=f"{kb_name}-id")
                logger.info("✅ SSM parameter deleted successfully")
            except Exception as e:
